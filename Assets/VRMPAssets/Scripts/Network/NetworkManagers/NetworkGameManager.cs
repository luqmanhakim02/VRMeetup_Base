...
namespace CustomMultiplayer
{
    ...
    public class NetworkGameManager : NetworkBehaviour
    {
        ...
        static NetworkGameManager s_Instance;
        ...
        /// Lobby Manager handles the Lobby and Relay work between players.
        /// </summary>
        public AuthManagerNew authenticationManager => m_AuthenticationManager;  // Public property to access the private field.
        ...
        protected virtual async void Awake()
        {
            ...
            if (s_Instance != null)
            {
                Utils.Log($"{k_DebugPrepend}Duplicate NetworkGameManager found, destroying.", 2);
                Destroy(gameObject);
                return;
            }
            s_Instance = this;
            ...
            if (TryGetComponent(out m_LobbyManager))
            {
                ...
                {
                    Utils.Log($"{k_DebugPrepend}AuthManagerNew is not found in the scene.", 1);
                    enabled = false;
                    return;
                }
            ...
            {
                Utils.Log($"{k_DebugPrepend}Missing LobbyManager, Disabling Component", 2);
                enabled = false;
                return;
            }
            ...
            {
                // Update connection state.
                Utils.Log($"{k_DebugPrepend}Authenticated successfully with PlayerId: {AuthenicationId}");
