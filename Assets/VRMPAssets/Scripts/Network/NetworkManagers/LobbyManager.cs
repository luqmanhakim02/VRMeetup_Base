    ...
    public class LobbyManager : MonoBehaviour
    {
        ...
        public async Task<bool> RemovePlayerFromLobby(string playerId)
        {
            ...
            try
            {
                if (m_ConnectedLobby != null)
                {
                    Utils.Log($"{k_DebugPrepend}RemovePlayerFromLobby called with playerId: {playerId}");
                    Utils.Log($"{k_DebugPrepend}Current Lobby HostId: {m_ConnectedLobby.HostId}");
                    ...
                    {
                        //Remove from lobby
                        Utils.Log($"{k_DebugPrepend}Player is not host. Removing from lobby.");
